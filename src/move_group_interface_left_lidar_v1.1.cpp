...
*/
  ...
  std::vector<double> getCurrentJointState(const std::string& topic_name)
  {
    sensor_msgs::JointStateConstPtr received_state = ros::topic::waitForMessage<sensor_msgs::JointState>(topic_name, ros::Duration(0.0));
    if (!received_state) throw std::runtime_error("Joint state message capture failed");
    return received_state->position;
  }
...
int main(int argc, char** argv)
{
  ...
  ROS_INFO("Robot's start state as seen by /move_group node: "); 
...
// =============================================================================================
  ...
  // planning_scene_monitor pointer
  // sleep(3.0); has no effect in 'always' getting the correct joint_state through psm -> planning_scene -> currentState
  planning_scene_monitor::PlanningSceneMonitorPtr planning_scene_monitor (new 
                          planning_scene_monitor::PlanningSceneMonitor(robot_model_loader));
  ...
  *   Ref: https://answers.ros.org/question/302330/why-is-robotstate-not-the-same-in-different-nodes/ */  
  ...
  ROS_INFO_STREAM("Planning scene request from move_group: " << (success_plan_scene ? "Succeeded." : "Failed."));

  
  // If not explicitly defined, /planning_scene is started by default. /move_group uses monitored_planning_scene
  // planning_scene_monitor->startSceneMonitor(); // DEFAULT_PLANNING_SCENE_TOPIC = "/planning_scene"
  ...
  // As I understood the ROS community, LockedPlanningSceneRW or RO should be used to later modify the allowed collision matrix programmatically 
  ...
  // If collision objects were added by the user into the scene, it would ideally listed as follows:
  // planning_scene_monitor->getPlanningScene()->printKnownObjects(std::cout);

  /** @brief  If GetPlanningScene service not called previously, getting correct current state from LockedPlanningSceneRW object lps is really random.
  *   Getting the state like this seems to lag in getting the correct current state as seen by the move_group. 
  *   Thus, state sometimes only starts with all 0, which is wrong in comparison to what move_group.getCurrentState() receives. 
  *   For now, calling GetPlanningScene seemed to work all the time with >50 tests done on the emulator */
  robot_state::RobotState start_state_planning = lps->getCurrentState();
  ROS_INFO("Start state from PlanningScene::getCurrentState():  "); 
  start_state_planning.printStatePositions(std::cout); 

  // this function that subcribes to "/joint_states" topic seems to work reliably everytime this node is launched
  std::vector<double> start_positions_from_joint_states = getCurrentJointState("joint_states"); // call to helper function above
  ROS_INFO("Start position of robot joints as subscribed to /joint_states topic: "); 
  std::copy(start_positions_from_joint_states.begin(), start_positions_from_joint_states.end(), std::ostream_iterator<double>(std::cout, ", "));

/* Effort to get the correct state from PlanningScene::getCurrentState() => Does not work!
  // The code below does not seem to work in updating the actual state as seen by move_group.getCurrentState OR values sent by /joint_states topic
  // sleep(5.0); Waiting for the LockedPlanningSceneRW object lps to get correct state values did also not work. 
  lps->getCurrentStateNonConst().update();
  start_state_planning = lps->getCurrentStateNonConst();
  ROS_INFO("Updated start state from PlanningScene::getCurrentState():  "); 
  start_state_planning.printStatePositions(std::cout); 
*/

  /** @brief Most likely ls->getPlanningSceneDiffMsg(diff_scene); fills in a full robot state but does not know about the attached object. 
  *   Adding diff_scene.robot_state.is_diff = true; after that statement should keep the attached object around. 
  *   Ref: https://answers.ros.org/question/352656/disabling-collision-in-allowedcollisionmatrix-removes-attached-object-from-the-scene/ */
