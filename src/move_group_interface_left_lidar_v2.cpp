  ...
  bool is_collision_matrix_checked = true;
...
*/
  ...
  // ******************************
  // Receive the recent robot state from planning_scene_monitor_
  robot_state::RobotState getCurrentRobotState(planning_scene_monitor::PlanningSceneMonitorPtr planning_scene_monitor_)
  {
    ...
    // success_plan_scene = planning_scene_monitor->requestPlanningSceneState(PLANNING_SCENE_SERVICE);
    // ROS_INFO_STREAM("Planning scene request from move_group: " << (success_plan_scene ? "SUCCESS." : "FAIL."));

    planning_scene_monitor::LockedPlanningSceneRW ps(planning_scene_monitor_);
    ps->getCurrentStateNonConst().update();
    robot_state::RobotState cur_rob_state = ps->getCurrentState();

    //if you want to modify it
    //planning_scene::PlanningScenePtr scene = ps->diff();
    //scene->decoupleParent();        

    return cur_rob_state;
  }
...
// ===========================================================================================================
  ...
  // Note: Inside this if statement only the related settings are done, set them back after actually moving to Home positon
  ...
  {   
    ...
    // In case planning_scene_monitor is declared in the beginning and defined locally here for a narrow scoped life
    // planning_scene_monitor::PlanningSceneMonitorPtr planning_scene_monitor (new planning_scene_monitor::PlanningSceneMonitor(robot_model_loader));  

    // ************ scope start ************
    { // LockedPlanningSceneRW within scope
      /** @brief The proper way is to get a fresh planning scene from the move_group node right when you need it. 
      ...
      *   Ref: https://answers.ros.org/question/302330/why-is-robotstate-not-the-same-in-different-nodes/ */ 
      ...
      // activate monitors locally OR keep the monitors deactivated
      if(is_local_scene_monitors_active) // by default false
      {
        ROS_INFO("Scene monitors from planning_scene_monitor are locally actived");
        planning_scene_monitor->startSceneMonitor("/move_group/monitored_planning_scene");
      ...
      acm.setEntry("environment", "left_tool0_mount", is_collision_pair_disabled);
      acm.setEntry("environment", "left_link_5", is_collision_pair_disabled);
      acm.setEntry("environment", "left_link_6", is_collision_pair_disabled);
  ...
  */
    ...
    // ************ scope start ************
    { // scope for enabling collision check
      success_plan_scene = planning_scene_monitor->requestPlanningSceneState(PLANNING_SCENE_SERVICE);
      ROS_INFO_STREAM("Planning scene request from move_group: " << (success_plan_scene ? "SUCCESS." : "FAIL."));
      ...
      // enable the collision checking between required pairs back
    ...
    // !! Important !! This code snippet below inserts another robot at Zero State into the scene, do not use it. 
    // The move group cannot do path planning since it thinks that the start point deviates from current robot state more than 0.01!
    // lps->getPlanningSceneDiffMsg(planning_scene);
    // planning_scene.robot_state.is_diff = false;  
    // planning_scene_interface.applyPlanningScene(planning_scene)       

      visual_tools.prompt("Collision matrix enabled to default again. Press 'Next' to continue!");
    }  
    // ************ scope end ************
  ...
  { // If the start state is not Zero State in the emulator
    // ************ scope start ************
    if (is_collision_matrix_checked)
    {
      success_plan_scene = planning_scene_monitor->requestPlanningSceneState(PLANNING_SCENE_SERVICE);
      ROS_INFO_STREAM("Planning scene request from move_group: " << (success_plan_scene ? "SUCCESS." : "FAIL."));
      ...
      visual_tools.prompt("Collision matrix observed after non-Zero start. Press 'Next' to continue!");
    }  
    // ************ scope end ************
  ...
  moveit::core::RobotState updated_robot_state = getCurrentRobotState(planning_scene_monitor);
